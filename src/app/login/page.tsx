  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row h-full w-full md:h-3/4 md:w-3/4 shadow-lg bg-white rounded-lg overflow-hidden">
        ...
        >
          <h2 className={`text-3xl md:text-4xl font-bold mb-4 md:mb-6 ${isSmallScreen ? 'text-white' : 'text-cyan-600'}`}>Log in</h2>
          <form onSubmit={handleSubmit} className="w-full max-w-xs md:max-w-md relative">
            <label htmlFor="email" className={`block mb-2 font-bold ${isSmallScreen ? 'text-white' : 'text-cyan-600'}`}>Email</label>
            <input
              type="email"
              id="email"
              value={email}
            ...
            />
            <label htmlFor="password" className={`block mb-2 font-bold ${isSmallScreen ? 'text-white' : 'text-cyan-600'}`}>Password</label>
            <input
              type="password"
              id="password"
              value={password}
              ...
              className="w-full p-2 md:p-3 mb-4 md:mb-6 border border-cyan-600 rounded text-black"
            />
            <div className={`flex items-center mb-4 md:mb-6 font-bold ${isSmallScreen ? 'text-white' : 'text-cyan-600'}`}>
              <input
                type="checkbox"
                id="remember"
                className="mr-2"
            ...
            <a href="#" className={`block mt-2 md:mt-4 text-center ${isSmallScreen ? 'text-white' : 'text-pink-500'}`}>Forgot Password?</a>
          </form>
          {isSmallScreen && (
        ...
        <div
          ...
          style={{
            backgroundImage: 'url(https://res.cloudinary.com/dduiqwdtr/image/upload/v1723017958/Hexerve%20website%20assets/zgrhyym7m4lzazgpq0wk.jpg)',
            backgroundSize: 'cover',
            backgroundPosition: 'center',
          }}
        >
          ...
          <div className="relative z-10">
            <h1 className="text-white text-5xl md:text-7xl font-bold text-center">AI-Driven IT Solutions</h1>
          </div>
        </div>
      </div>
    </div>
